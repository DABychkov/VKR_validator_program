...
def find_keywords_section(paragraphs: list[str]) -> str | None:
    """
    Ищет БЛОК ключевых слов.
    преимущественно капсом (>=80% букв в верхнем регистре)
    не начинается с "1.1 ", "1.2 " и т.д.
...
def check_keywords_format(keywords_block: str) -> dict[str, bool]:
    """
    Проверяет формат ключевых слов.
    все буквы в каждой строке прописные (капс)
    большинство строк содержат запятые как разделители
    последняя строка не заканчивается точкой
    не более 4 строк и без признаков переноса слова
    """
    results = {
        "is_uppercase": True,
        "has_commas": True,
